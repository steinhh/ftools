...
def _cutout_spectrum(src_spec, SG_dopp, velocity_array, velocity_range, islit):
    """
    Cutout a given spectrum corresponding to the velocity range per slit.
    ...
    `xarray.Dataset`
        Cut out spectrum.
    """
    if "slit" not in velocity_array.dims:
        velocity_array = reshape_x2rs(velocity_array)
    return src_spec.flux * xr.where(
        np.abs(SG_dopp.sel(slit=islit) - velocity_array.sel(slit=islit)) <= velocity_range,
        1,
        0,
    )


def _vet_new_local_max(spec_vrange, spec_largervrange):
    """
    Check if the local maximum remains consistent between two spectra.
    A local maximum is deemed consistent if:

        ``Max[spec_vrange] == Max[spec_largervrange]``

    where we expect the two spectrum to be the local maxima across the velocity range
    and velocity range+/- few pixels.

    Parameters
    ----------
    spec_vrange : `xarray.Dataset`
        Spectrum 1.
    spec_largervrange : `xarray.Dataset`
        Spectrum 2.

    Returns
    -------
    `xarray.Dataset`
        Return a spectrum masking out points which do not show consistent local maximum.
    """
    max_loc_vrange = spec_vrange.argmax(dim="SG_xpixel")
    max_loc_largervrange = spec_largervrange.argmax(dim="SG_xpixel")
    diff = np.abs(max_loc_vrange - max_loc_largervrange)
    return spec_vrange * xr.where(
        diff == 0,
        1,
        0,
    )
...
def _split_spec_slitwise(dst_spec, src_spec, velocity_range, velocity_array, SG_dopp, nslit, flag_pix_slac=0):
    ...
    for islit in range(nslit):
        ...
        if "slit" in src_spec[0].coords:
            tmp = src_spec[0].sel(slit = islit)
        else:
            tmp = src_spec[0]
        spec_vrange = _cutout_spectrum(tmp, SG_dopp, velocity_array, velocity_range, islit)
        if flag_pix_slac:
            dv = np.median(np.gradient(SG_dopp, axis=-1)) * flag_pix_slac
            spec_largervrange = _cutout_spectrum(tmp, SG_dopp, velocity_array, velocity_range + dv, islit)
            spec_vrange = _vet_new_local_max(spec_vrange, spec_largervrange)
        dst_spec[0]["flux"].loc[{"slit": islit}] = spec_vrange
        if "slit" in src_spec[1].coords:
            tmp = src_spec[1].sel(slit = islit)
        else:
            tmp = src_spec[1]
        spec_vrange_noise = _cutout_spectrum(tmp, SG_dopp, velocity_array, velocity_range, islit)
        dst_spec[1]["flux"].loc[{"slit": islit}] = spec_vrange_noise
    return dst_spec
...
def _preprocess_spec_for_fitting(input_spectrum, input_noise, difference_npix, npix, max_value, max_noise, max_indices, SG_dopp, nslit, npix_left=None, npix_right=None):
    ...
    """
    all_inside =  xr.where(
        np.abs(difference_npix) <= npix,
        1,
        np.nan,
    )
    less_than_npix  = xr.where(all_inside.sum("SG_xpixel", skipna = True) <2*npix+1, 1, np.nan)
    all_inside_npixp1 = xr.where(
        np.abs(difference_npix) <= npix+1,
        1,
        np.nan,
    )
    all_inside_npixp1 = all_inside_npixp1 * less_than_npix
    # Spectra may be at the left or right edge, and may not have exactly +/- npix.
    # This ensures the number of points in spectrum is consistent across all spectra
    all_inside = xr.where(all_inside == 1, all_inside, all_inside_npixp1)
    output_spectrum = input_spectrum.copy(deep=True)
    output_noise = input_noise.copy(deep=True)
    output_spectrum["flux"] = output_spectrum["flux"] * all_inside
    output_noise["flux"] = output_noise["flux"] * all_inside
    if npix_left is not None and npix_right is not None:
        mask_left = xr.where(difference_npix >= -npix_left, 1, np.nan)
        mask_right = xr.where(difference_npix <= npix_right, 1, np.nan)
    ...
    tmp_arr = xr.zeros_like(max_indices)
    for islit in range(nslit):
        # Taking up the index starting from 0.
        tmp_arr.loc[{"slit": islit}] = SG_dopp.sel(slit=islit)[max_indices.sel(slit=islit)]
        ...
        if method=='caruana_pytorch':
           ...
           # Build Jacobian matrix -  (batch_size, 3, 3) for [sigma, mu, amplitude] w.r.t. [A, B, C]
            ...
            if serial:
                ...
                for i, args in enumerate(list_fit):
                    if method == 'mpfit':
...
def calculate_moments_gaussian(spectrum, response, *, line=None, **kwargs):
    ...
    **kwargs : dict, additional arguments
    ...
    print()
    print("----- Input to Gaussian fitting -----")
    print()
        ...
        if "channel" in spectrum_observable.flux.dims:
            ...
            for l1,l2 in zip(spec_channels.data,line_channel_key):
                ...
                if isinstance(tmp, tuple):
                    ...
                    moments_observable.append(tmp[0].assign_coords({"line": l2}))
                    fitted_spec.append(tmp[1])
                else:
                    mom = tmp
