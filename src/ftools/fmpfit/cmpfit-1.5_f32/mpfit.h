...
struct mp_par_struct {
  ...
  float limits[2];  /* lower/upper limit boundary value */

  char *parname;    /* Name of parameter, or 0 for none */
  float step;       /* Step size for finite difference */
  float relstep;    /* Relative step size for finite difference */
  ...
  int deriv_debug;  /* Derivative debug mode: 1 = Yes; 0 = No;
		       ...
		       (0, 1, -1, or 2).
		    */
  float deriv_reltol; /* Relative tolerance for derivative debug
			  printout */
  float deriv_abstol; /* Absolute tolerance for derivative debug
			  printout */
};
...
struct mp_config_struct {
  /* NOTE: the user may set the value explicitly; OR, if the passed
     ...
     mpfit(). */
  float ftol;    /* Relative chi-square convergence criterium Default: 1e-10 */
  float xtol;    /* Relative parameter convergence criterium  Default: 1e-10 */
  float gtol;    /* Orthogonality convergence criterium       Default: 1e-10 */
  float epsfcn;  /* Finite derivative step size               Default: MP_MACHEP0 */
  float stepfactor; /* Initial step bound                     Default: 100.0 */
  float covtol;  /* Range tolerance for covariance calculation Default: 1e-14 */
...
struct mp_result_struct {
  float bestnorm;     /* Final chi^2 */
  float orignorm;     /* Starting value of chi^2 */
  ...
  float *resid;       /* Final residuals
			  nfunc-vector, or 0 if not desired */
  float *xerror;      /* Final parameter uncertainties (1-sigma)
			  npar-vector, or 0 if not desired */
  float *covar;       /* Final parameter covariance matrix
...
typedef int (*mp_func)(int m, /* Number of functions (elts of fvec) */
		       ...
		       float *x,      /* I - Parameters */
		       float *fvec,   /* O - function values */
		       float **dvec,  /* O - function derivatives (optional)*/
...
/* Float (single) precision numeric constants */
#define MP_MACHEP0 1.19209290e-07f
#define MP_DWARF   1.17549435e-38f
#define MP_GIANT   3.40282347e+38f

#define MP_RDWARF  (sqrtf(MP_DWARF*1.5f)*10.0f)
#define MP_RGIANT  (sqrtf(MP_GIANT)*0.1f)


/* External function prototype declarations */
extern int mpfit(mp_func funct, int m, int npar,
		 float *xall, mp_par *pars, mp_config *config, 
		 void *private_data, 
		 mp_result *result);
