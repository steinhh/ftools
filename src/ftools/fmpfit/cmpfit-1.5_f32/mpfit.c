...
/* Main mpfit library routines (float precision) 
   $Id$
 */
...
static int mp_fdjac2(mp_func funct,
	      int m, int n, int *ifree, int npar, float *x, float *fvec,
	      float *fjac, int ldfjac, float epsfcn,
	      float *wa, void *priv, int *nfev,
	      float *step, float *dstep, int *dside,
	      int *qulimited, float *ulimit,
	      int *ddebug, float *ddrtol, float *ddatol,
	      float *wa2, float **dvecptr);
static void mp_qrfac(int m, int n, float *a, int lda, 
	      int pivot, int *ipvt, int lipvt,
	      float *rdiag, float *acnorm, float *wa);
static void mp_qrsolv(int n, float *r, int ldr, int *ipvt, float *diag,
	       float *qtb, float *x, float *sdiag, float *wa);
static void mp_lmpar(int n, float *r, int ldr, int *ipvt, int *ifree, float *diag,
	      float *qtb, float delta, float *par, float *x,
	      float *sdiag, float *wa1, float *wa2);
static float mp_enorm(int n, float *x);
static float mp_dmax1(float a, float b);
static float mp_dmin1(float a, float b);
static int mp_min0(int a, int b);
static int mp_covar(int n, float *r, int ldr, int *ipvt, float tol, float *wa);
...
*     float *xall   - array of n initial parameter values
...
*	  interval (.1,100.f). 100.f is a generally recommended value.
*
...
*
*     argonne national laboratory. minpack project. march 1980.f
...
int mpfit(mp_func funct, int m, int npar,
	  float *xall, mp_par *pars, mp_config *config, void *private_data, 
	  mp_result *result)
{
  mp_config conf;
  ...
  int ij,jj,l;
  float actred,delta,dirder,fnorm,fnorm1,gnorm, orignorm;
  float par,pnorm,prered,ratio;
  float sum,temp,temp1,temp2,temp3,xnorm, alpha;
  static float one = 1.0f;
  static float p1 = 0.1f;
  static float p5 = 0.5f;
  static float p25 = 0.25f;
  static float p75 = 0.75f;
  static float p0001 = 1.0e-4f;
  static float zero = 0.0f;
  int nfev = 0;

  float *step = 0, *dstep = 0, *llim = 0, *ulim = 0;
  int *pfixed = 0, *mpside = 0, *ifree = 0, *qllim = 0, *qulim = 0;
  int *ddebug = 0;
  float *ddrtol = 0, *ddatol = 0;

  float *fvec = 0, *qtf = 0;
  float *x = 0, *xnew = 0, *fjac = 0, *diag = 0;
  float *wa1 = 0, *wa2 = 0, *wa3 = 0, *wa4 = 0;
  float **dvecptr = 0;
  int *ipvt = 0;
  ...
  mp_malloc(step,  float, npar);
  mp_malloc(dstep, float, npar);
  mp_malloc(mpside, int, npar);
  mp_malloc(ddebug, int, npar);
  mp_malloc(ddrtol, float, npar);
  mp_malloc(ddatol, float, npar);
  if (pars) for (i=0; i<npar; i++) {
    step[i] = pars[i].step;
  ...
  if (pars) {
    ...
    mp_malloc(ulim, float, nfree);
    mp_malloc(llim, float, nfree);
  ...
  mp_malloc(fvec, float, m);
  mp_malloc(qtf, float, nfree);
  mp_malloc(x, float, nfree);
  mp_malloc(xnew, float, npar);
  mp_malloc(fjac, float, m*nfree);
  ldfjac = m;
  mp_malloc(diag, float, npar);
  mp_malloc(wa1, float, npar);
  mp_malloc(wa2, float, m); /* Maximum usage is "m" in mpfit_fdjac2() */
  mp_malloc(wa3, float, npar);
  mp_malloc(wa4, float, m);
  mp_malloc(ipvt, int, npar);
  mp_malloc(dvecptr, float *, npar);
 ...
 OUTER_LOOP:
  ...
  if (fnorm != zero) {
	...
	gnorm = mp_dmax1(gnorm,fabsf(sum/wa2[l]));
      }
      jj += m;
    }
  }
 ...
 L200:
  ...
  if (qanylim == 0) {
    ...
    for (j=0; j<nfree; j++) {
      int lpegged = (qllim[j] && (x[j] <= llim[j]));
      int upegged = (qulim[j] && (x[j] >= ulim[j]));
      int dwa1 = fabsf(wa1[j]) > MP_MACHEP0;
  ...
  temp2 = (sqrtf(alpha*par)*pnorm)/fnorm;
   ...
   */
  if ((fabsf(actred) <= conf.ftol) && (prered <= conf.ftol) && 
      (p5*ratio <= one) ) {
    info = MP_OK_CHI;
  }
  ...
  if ((fabsf(actred) <= conf.ftol) && (prered <= conf.ftol) && (p5*ratio <= one)
      && ( info == 2) ) {
    info = MP_OK_BOTH;
  }
  if (info != 0) {
  ...
  if ((fabsf(actred) <= MP_MACHEP0) && (prered <= MP_MACHEP0) && (p5*ratio <= one) ) {
    info = MP_FTOL;
  }
 ...
 L300:
  ...
  if (result && (result->covar || result->xerror)) {
    ...
    if (result->xerror) {
      ...
      for (j=0; j<nfree; j++) {
	float cc = fjac[j*ldfjac+j];
	if (cc > 0) result->xerror[ifree[j]] = sqrtf(cc);
      }
    }
  }      
...
*
*	  the value of iflag should not be changed by fcn unless
*	  the user wants to terminate execution of fdjac2.f
*	  in this case set iflag to a negative integer.
*
*	m is a positive integer input variable set to the number
*	  of functions.
...
*	  the execution of fdjac2.f see description of fcn.
*
*	epsfcn is an input variable used in determining a suitable
*	  step length for the forward-difference approximation. this
*	  approximation assumes that the relative errors in the
...
*/
  ...
  if (has_numerical_deriv) for (j=0; j<n; j++) {  /* Loop thru free parms */
    ...
    float dr = ddrtol[ifree[j]], da = ddatol[ifree[j]];
    ...
    if (dstep && dstep[ifree[j]] > 0) h = fabsf(dstep[ifree[j]]*temp);
    if (h == zero)                    h = eps;
	...
	for (i=0; i<m; i++, ij++) {
	  ...
	  if ((da == 0 && dr == 0 && (fjold != 0 || fjac[ij] != 0)) ||
	      ((da != 0 || dr != 0) && (fabsf(fjold-fjac[ij]) > da + fabsf(fjold)*dr))) {
	    printf("   %10d %10.4fg %10.4fg %10.4fg %10.4fg %10.4fg\n", 
 ...
 DONE:
  ...
  return 0; 
  /*
   *     last card of subroutine fdjac2.f
   */
}
...
static 
void mp_qrfac(int m, int n, float *a, int lda, 
	      int pivot, int *ipvt, int lipvt,
	      float *rdiag, float *acnorm, float *wa)
{
/*
...
static 
void mp_qrsolv(int n, float *r, int ldr, int *ipvt, float *diag,
	       float *qtb, float *x, float *sdiag, float *wa)
{
/*
*     **********
...
*/
  ...
  float cosx,cotan,qtbpj,sinx,sum,tanx,temp;
  static float zero = 0.0f;
  static float p25 = 0.25f;
  static float p5 = 0.5f;
  ...
  for (j=0; j<n; j++) {
	...
	if (sdiag[k] == zero)
	  continue;
	kk = k + ldr * k;
	if (fabsf(r[kk]) < fabsf(sdiag[k]))
	  {
	    cotan = r[kk]/sdiag[k];
	    sinx = p5/sqrtf(p25+p25*cotan*cotan);
	    cosx = sinx*cotan;
	  }
	else
	  {
	    tanx = sdiag[k]/r[kk];
	    cosx = p5/sqrtf(p25+p25*tanx*tanx);
	    sinx = cosx*tanx;
	  }
	/*
...
static 
void mp_lmpar(int n, float *r, int ldr, int *ipvt, int *ifree, float *diag,
	      float *qtb, float delta, float *par, float *x,
	      float *sdiag, float *wa1, float *wa2) 
{
  /*     **********
   ...
   *
   *	    (dxnorm-delta) .le. 0.1f*delta ,
   *
   *     or par is positive and
   *
   *	    fabsf(dxnorm-delta) .le. 0.1f*delta .
   *
   *     this subroutine completes the solution of the problem
   *     if it is provided with the necessary information from the
   *     qr factorization, with column pivoting, of a. that is, if
  ...
  float dxnorm,fp,gnorm,parc,parl,paru;
  float sum,temp;
  static float zero = 0.0f;
  /* static float one = 1.0f; */
  static float p1 = 0.1f;
  static float p001 = 0.001f;
 ...
 L150:
   ...
   *	 is zero or the number of iterations has reached 10.f
   */
...
float mp_enorm(int n, float *x) 
{
   ...
   *
   *	float precision function enorm(n,x)
   *
   *     where
   *
   ...
   */
  int i;
  float agiant,floatn,s1,s2,s3,xabs,x1max,x3max;
  float ans, temp;
  float rdwarf = MP_RDWARF;
  float rgiant = MP_RGIANT;
  static float zero = 0.0f;
  static float one = 1.0f;
...
static 
float mp_dmax1(float a, float b) 
{
  if (a >= b)
    return(a);
  else
...
static 
float mp_dmin1(float a, float b)
{
  if (a <= b)
    return(a);
  else
...
c
c           fabsf(r(l,l)) .gt. tol*fabsf(r(1,1)) ,
c
...
c
c     argonne national laboratory. minpack project. august 1980.f
...
static 
int mp_covar(int n, float *r, int ldr, int *ipvt, float tol, float *wa)
{
  int i, ii, j, jj, k, l;
  int kk, kj, ji, j0, k0, jj0;
  int sing;
  float one = 1.0f, temp, tolr, zero = 0.0f;
