          ...
          mp_result *result)
{
    ...
    static mp_real p75 = 0.75;
    static mp_real p0001 = 1.0e-4;
    static mp_real zero = 0.0;
    int nfev = 0;

    mp_real *step = 0, *dstep = 0, *llim = 0, *ulim = 0;
    int *pfixed = 0, *mpside = 0, *ifree = 0, *qllim = 0, *qulim = 0;
    int *ddebug = 0;
    mp_real *ddrtol = 0, *ddatol = 0;

    mp_real *fvec = 0, *qtf = 0;
    mp_real *x = 0, *xnew = 0, *fjac = 0, *diag = 0;
    mp_real *wa1 = 0, *wa2 = 0, *wa3 = 0, *wa4 = 0;
    mp_real **dvecptr = 0;
    int *ipvt = 0;
    ...
    mp_malloc(wa2, mp_real, m); /* Maximum usage is "m" in mpfit_fdjac2() */
    mp_malloc(wa3, mp_real, npar);
    mp_malloc(wa4, mp_real, m);
    mp_malloc(ipvt, int, npar);
    mp_malloc(dvecptr, mp_real *, npar);
                     ...
                     mp_real *sdiag, mp_real *wa1, mp_real *wa2)
{
     ...
     *
     *	x is an output array of length n which contains the least
     *	  squares solution of the system a*x = b, mp_sqrt(par)*d*x = 0,
     *	  for the output par.
     *
     *	sdiag is an output array of length n which contains the
     *	  diagonal elements of the upper triangular matrix s.
...
static mp_real mp_enorm(int n, mp_real *x)
{
     ...
     *
     *	mp_real precision function enorm(n,x)
     *
     *     where
     *
...
static int mp_covar(int n, mp_real *r, int ldr, int *ipvt, mp_real tol, mp_real *wa)
{
    ...
    int sing;
    mp_real one = 1.0, temp, tolr, zero = 0.0;
...
/* Helper: Invert a 3x3 symmetric matrix using cofactor expansion (Cramer's rule)
 * This is more numerically stable than Gauss-Jordan for ill-conditioned 3x3 matrices.
 * Returns determinant (0 if singular). */
static mp_real mp_invert_3x3_symmetric(mp_real *A, mp_real *Ainv)
{
    mp_real a = A[0], b = A[1], c = A[2];
    mp_real d = A[4], e = A[5];
    mp_real f = A[8];
    /* A = | a b c |   (symmetric, so A[3]=b, A[6]=c, A[7]=e)
     *     | b d e |
     *     | c e f |
     */
    ...
    if (mp_fabs(det) < 1e-30)
    {
        /* Singular matrix */
        return 0.0;
    }

    mp_real inv_det = 1.0 / det;

    /* Inverse = adjugate / det (adjugate = transpose of cofactor matrix) */
    Ainv[0] = C00 * inv_det;
    Ainv[1] = C01 * inv_det;
    Ainv[2] = C02 * inv_det;
    Ainv[3] = C01 * inv_det; /* symmetric */
    Ainv[4] = C11 * inv_det;
    Ainv[5] = C12 * inv_det;
    Ainv[6] = C02 * inv_det; /* symmetric */
    Ainv[7] = C12 * inv_det; /* symmetric */
    Ainv[8] = C22 * inv_det;
                            ...
                            mp_real *xerror_scipy)
{
    ...
    inv = (mp_real *)malloc(npar * npar * sizeof(mp_real));
    row_scale = (mp_real *)malloc(npar * sizeof(mp_real));
    dvec = (mp_real **)malloc(npar * sizeof(mp_real *));

    if (!fjac_full || !hess || !inv || !row_scale || !dvec)
    {
        goto cleanup_scipy;
    }
    ...
    /* For 3x3 matrices, use explicit cofactor inversion (more robust) */
    if (npar == 3)
    {
        mp_real det = mp_invert_3x3_symmetric(hess, inv);
        if (mp_fabs(det) > 1e-30)
        {
            /* Successfully inverted - extract errors */
            for (i = 0; i < npar; i++)
            {
                var = inv[i * npar + i];
        ...
        /* Fall through to Gauss-Jordan if cofactor method fails */
    }

    /* For larger matrices or if cofactor failed, use scaled Gauss-Jordan */
    /* Initialize inv as identity */
    for (i = 0; i < npar; i++)
    {
        for (j = 0; j < npar; j++)
        {
            inv[i * npar + j] = (i == j) ? one : zero;
        }
    }

    /* Compute row scaling factors for equilibration */
    for (i = 0; i < npar; i++)
    {
        max_val = zero;
        for (j = 0; j < npar; j++)
        {
            tmp = mp_fabs(hess[i * npar + j]);
            if (tmp > max_val)
                max_val = tmp;
        }
        row_scale[i] = (max_val > MP_MACHEP0) ? max_val : one;
    }

    /* Gauss-Jordan with scaled partial pivoting */
    for (col = 0; col < npar; col++)
    {
        /* Find pivot using scaled values */
        pivot = col;
        max_val = mp_fabs(hess[col * npar + col]) / row_scale[col];
        for (row = col + 1; row < npar; row++)
        {
            tmp = mp_fabs(hess[row * npar + col]) / row_scale[row];
            if (tmp > max_val)
            {
                max_val = tmp;
                pivot = row;
        ...
        if (pivot != col)
        {
            for (k = 0; k < npar; k++)
            {
                ...
                tmp = inv[col * npar + k];
                inv[col * npar + k] = inv[pivot * npar + k];
                inv[pivot * npar + k] = tmp;
            }
            tmp = row_scale[col];
            row_scale[col] = row_scale[pivot];
            row_scale[pivot] = tmp;
        }

        /* Scale pivot row */
        pivot_val = hess[col * npar + col];
        if (mp_fabs(pivot_val) > MP_MACHEP0 * 100)
        {
            for (k = 0; k < npar; k++)
            {
                hess[col * npar + k] /= pivot_val;
                inv[col * npar + k] /= pivot_val;
            }
            ...
            for (row = 0; row < npar; row++)
            {
                if (row != col)
                {
                    factor = hess[row * npar + col];
                    for (k = 0; k < npar; k++)
                    {
                        hess[row * npar + k] -= factor * hess[col * npar + k];
                        inv[row * npar + k] -= factor * inv[col * npar + k];
                    }
                }
            }
        }
    ...
    /* Extract errors from diagonal of inv */
    for (i = 0; i < npar; i++)
    {
        var = inv[i * npar + i];
        xerror_scipy[i] = (var > zero && var < 1e29) ? mp_sqrt(var) * scale_factor : zero;
    }
