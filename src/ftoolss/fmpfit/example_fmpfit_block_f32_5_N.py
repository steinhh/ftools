...
# -*- coding: utf-8 -*-
"""
Example usage of fmpfit_f32_block extension - Minimal data case
...
Fits N spectra in a single block call using float32 precision.

Usage:
    python example_fmpfit_block_f32_5_N.py [N]
...
try:
    ...
    fmpfit_f32_block_pywrap = fmpfit_module.fmpfit_f32_block_pywrap

# Parse command line argument for number of spectra
N = int(sys.argv[1]) if len(sys.argv) > 1 else 1000

print("=" * 70)
print(f"FMPFIT BLOCK FLOAT32 Minimal Data Example: 5 pixels, {N} spectra")
print("=" * 70)
...
x_1d = np.linspace(-fwhm/2, fwhm/2, mpoints, dtype=np.float32)

p0_1d = np.array([2.0, 0.0, 1.0], dtype=np.float32)  # Initial guesses
bounds_1d = np.array([[0.0, 10.0], [-2.0, 2.0], [0.1, 5.0]], dtype=np.float32)
...
error = np.sqrt(np.maximum(y, 1.0)).astype(np.float32)  # Avoid division by zero
...
result = fmpfit_f32_block_pywrap(
...
print("\nThis example uses float32 precision block fitting, which processes")
print("all spectra in a single C call with the GIL released.")
print("Typical throughput: ~125,000 spectra/s (~8 us/spectrum) for 5-point fits.")
print("=" * 70)
