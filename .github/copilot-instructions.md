...
Use replace_string_in_file/multi_replace_string_in_file instead of Python scripts for changes, since the former allows user to see and approve each change with keep/undo interface. For scattered individual changes, make them one at a time (or in contiguous groups) so each can be individually reviewed. Never use bulk Python scripts that bypass the VSCode accept/reject interface for file modifications. For complex logic that cannot be easily handled with replace_string_in_file, use Python scripts for analysis and planning (outputting what changes should be made, e.g., as oldString/newString pairs or diffs), then execute those changes using replace_string_in_file/multi_replace_string_in_file so the user can review each change with the keep/undo interface. Do NOT delete the python scripts after use, since that requires approval in VSCode.
